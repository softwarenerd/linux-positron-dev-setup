#!/usr/bin/env bash
#
# setup-fedora.sh — Configure a Fedora/RHEL-family (Fedora, RHEL, Rocky, Alma,
# CentOS Stream, ...) machine for Positron development.
#
...
#
# Usage:
#   ./setup-fedora.sh          run the setup steps
#   ./setup-fedora.sh --undo   revert what a previous run installed/created
#
...
# Fedora/RHEL-repo names for the same libraries setup-debian.sh installs (e.g.
# Debian's libsecret-1-dev is libsecret-devel here).
PACKAGES=(
  gcc
  gcc-c++
  make
...
# pkg_installed <pkg>: true if the rpm package is currently installed.
pkg_installed() {
...
# versions available for the release the developer chose. (We use `makecache`
# rather than `check-update`, which returns exit code 100 when updates exist and
# would trip `set -e`.)
refresh_index() {
...
maybe_upgrade() {
  ...
  if confirm "Upgrade installed packages to the latest within the current release?"; then
    log "upgrading packages (dnf upgrade)..."
    sudo dnf upgrade -y
  else
...
# Assumes refresh_index has already run, so the metadata is current. dnf is
...
install_python() {
  ...
  # Packages needed to compile CPython from source (pyenv's "suggested build
  # environment" for Fedora/RHEL). Only the ones not already present are
  # recorded, so --undo removes just what we added.
  local build_deps=(
...
# arm64 from Microsoft. Downloads the .rpm into ~/Downloads, then installs it with
# dnf (which resolves any dependencies). Idempotent — skips if the code package is
...
install_vscode() {
  ...
  # Check the dpkg/rpm package, not `code` on PATH: in a VS Code remote/SSH
  # session the server ships its own `code` CLI shim, so `have code` gives a
  # false positive even when the package isn't installed.
  if pkg_installed code; then
    log "Visual Studio Code already installed (code package present); skipping."
    return 0
  fi

  # curl fetches the .rpm over HTTPS; make sure it's present (record only if we
  ...
  # a versioned .rpm (e.g. code-1.126.0-....aarch64.rpm); -L follows the redirect
  ...
  ( cd "$dl" && curl -fSL -OJ "https://code.visualstudio.com/sha/download?build=stable&os=linux-rpm-arm64" )

  # Pick the VS Code .rpm to install. Normally there's exactly one, but if older
  ...
  local rpm
  rpm="$(ls "$dl"/code-*.rpm 2>/dev/null | sort -V | tail -n1)"
  if [ -z "$rpm" ]; then
    log "WARNING: no VS Code .rpm found in $dl after download; skipping install."
    return 0
  fi
...
undo() {
  ...
  if [ "${#pkgs[@]}" -gt 0 ]; then
    log "removing ${#pkgs[@]} packages that setup installed..."
